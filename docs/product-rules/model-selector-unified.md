...
- 模型名超长:max-width 截断 + hover title 全名。正常工具栏 pill 按内容展开，整体上限 320px（不超过父容器）；窄 / 超窄仍为 148px / 64px。避免超长 pill 通过 MorphPopover 的锚点宽度下限撑大展开面板。
...
- 宽度自适应:`max-content`,min 300px / max min(460px, 100vw-48px)；优先保持紧凑，超出可用宽度的模型名显示省略号，hover title 保留全名，不为显示完整名称继续加宽（2026-09-17 截断原则；2026-09-21 将上限从 420px 调整为 460px，以完整显示「Codex GPT-6 Astra」量级的名称）。字段形态仍绑定字段宽度；视口和锚点可用空间不足时继续收缩。
- 行(双行):
...
- **远程收藏按被控电脑共享**：Desktop 与 Mobile 连接同一台电脑时读取和修改该电脑当前账号的既有收藏。各被控电脑独立，不将控制端的本机收藏套用或自动合并到远端。收藏以单条新增、条件编辑、条件删除同步，等持久化确认后反馈成功；并发修改同一副本时拒绝旧快照覆盖，重读后再编辑。断线、账号切换、旧版不支持时不得声称已同步；不自动重试写操作，也不影响会话和连接恢复。
