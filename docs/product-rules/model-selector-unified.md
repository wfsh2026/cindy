...
- 宽度自适应:`max-content`,min 300px / max min(420px, 100vw-48px)；优先保持紧凑，超出可用宽度的模型名显示省略号，hover title 保留全名，不为显示完整名称额外扩宽（2026-09-17 用户反馈修订）。字段形态仍绑定字段宽度；视口和锚点可用空间不足时继续收缩。
- 行(双行):
