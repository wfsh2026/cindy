 ...
 *
 * 在线清单优先；网络失败后允许本区持久缓存、随包完整清单兜底。
 * JSON/schema/非法 URL/区域不匹配与永久 HTTP 配置错误仍阻断，绝不逐字段合并。
 * 缓存重新严格解析并按目标区域校验受信任域，来源地址变化即失效。
 * 备用入口是构建期信任锚点，不能由下载的清单或缓存指定。
 * 更新端点与审核元数据保持完整快照语义，账号区域切换不改变构建更新身份。
