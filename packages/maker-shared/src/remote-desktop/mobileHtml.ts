...
export function remoteDesktopViewerHtml(
  ...
  *{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:${color(surface)};touch-action:none;overscroll-behavior:none}#stage{position:absolute;inset:0;overflow:hidden;z-index:0}#bg{position:absolute;inset:0;overflow:hidden;z-index:0;display:none}#bg-canvas{position:absolute;left:0;top:0;width:100%;height:100%;pointer-events:none}video,img{position:absolute;max-width:none;pointer-events:none;transform-origin:0 0}video{display:none;z-index:0}#cursor{position:absolute;z-index:3;width:18px;height:18px;border:2px solid ${color(foreground)};border-radius:50%;pointer-events:none;display:none;transform:translate(-50%,-50%)}
  /* Overscan the ambient layer so blur cannot expose a clear strip at screen edges. */
  #bg-canvas{filter:blur(16px);transform:scale(1.12);transform-origin:center;opacity:.72}
  #network-status{position:absolute;z-index:1;display:none;pointer-events:none;max-width:75%;white-space:pre-line;text-align:right;font-family:-apple-system,BlinkMacSystemFont,sans-serif;font-variant-numeric:tabular-nums;font-weight:400;color:var(--foreground)}
  ...
  video,img{border:0;outline:0}#image{z-index:2}img:not([src]){visibility:hidden}
  /* The JPEG stays beneath live video; cast one soft shadow, never two. */
  :root{--desktop-picture-shadow:0 0 10px rgba(0,0,0,.16)}
  #image,#image:not([src])~#video{box-shadow:var(--desktop-picture-shadow)}
