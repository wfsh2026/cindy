...
import { rewriteRoutedCustomToolsForUpstream } from "../../../upstream/responses/custom-tool-compat";
function convertedInputDescription(name) {
...
describe("routed custom-tool compatibility", () => {
    test.each([
        ["absent", undefined],
        ["true", true],
    ])("keeps apply_patch byte-identical when custom-tool support is %s", (_label, support) => {
        const raw = {
    ...
    ])("does not arm apply_patch repair under %s", (_label, toolChoice) => {
        const rewritten = rewriteRoutedCustomToolsForUpstream({
            tools: [
    ...
    ])("arms apply_patch repair when the selector authorizes it", toolChoice => {
        const rewritten = rewriteRoutedCustomToolsForUpstream({
            tools: [{ type: "custom", name: "apply_patch", description: "Apply a patch", format: { type: "text" } }],
            tool_choice: toolChoice,
        });
