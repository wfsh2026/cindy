...
import { compileCodeModeHelperInput } from "../../../upstream/responses/code-mode-helper-compat";
...
describe("code-mode helper compatibility", () => {
    test("exec_command arguments remain data when generated JavaScript runs", async () => {
        ...
        const source = compileCodeModeHelperInput(JSON.stringify({ cmd: command, workdir: "/tmp", yield_time_ms: 1_000 }), "exec_command");
        let received;
        let output;
    ...
    test("shell_command maps command to the nested exec cmd field", async () => {
        const source = compileCodeModeHelperInput(JSON.stringify({ command: "pwd", workdir: "/tmp" }), "shell_command");
        let received;
        const run = new AsyncFunction("tools", "text", source);
        await run({
            exec_command: async (args) => {
    ...
    test("apply_patch normalizes decorated outer delimiters before execution", async () => {
        ...
        let received;
        const run = new AsyncFunction("tools", "text", compileCodeModeHelperInput(JSON.stringify({ input: decorated }), "apply_patch"));
        await run({
            apply_patch: async (input) => {
                received = input;
                return "done";
