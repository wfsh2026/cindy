...
import { restoreRoutedCustomCalls, rewriteRoutedCustomToolsForUpstream } from "../../../upstream/responses/custom-tool-compat";
import { createRoutedNamespaceCallRestoreRewrite, restoreRoutedNamespaceCalls, restoreRoutedNamespaceCallsInJson, rewriteRoutedNamespaceToolsForUpstream, } from "../../../upstream/responses/namespace-tool-compat";
describe("Responses namespace tool compatibility", () => {
    ...
    describe("allowed_tools authorization is restricted by tool type", () => {
        ...
        test.each([
            ...
            expect(restored.value.namespace).toBeUndefined();
        });
        ...
        test.each([
            ...
            const build = (choice) => rewriteRoutedNamespaceToolsForUpstream({
    ...
    test("default and absent tool_choice keep every alias", () => {
        ...
        for (const choice of [undefined, "auto", "required"]) {
            const { aliases } = rewriteRoutedNamespaceToolsForUpstream(choice === undefined ? { tools } : { tools, tool_choice: choice });
    ...
    test("chooses the bare declaration regardless of which tool container comes first", () => {
        ...
        const flatten = (bodyTools, additionalTools) => {
            const rewritten = rewriteRoutedNamespaceToolsForUpstream({
                tools: bodyTools,
