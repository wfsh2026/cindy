...
import { isCompletePatchEnvelope, mayBecomePatchEnvelope, normalizeApplyPatchDelimiters, repairFreeformToolInput, } from "../../../upstream/responses/apply-patch-envelope";
import { compileCodeModeHelperInput, resolveCodeModeHelperName } from "../../../upstream/responses/code-mode-helper-compat";
...
describe("raw exec patch envelope recognition", () => {
    ...
    test("refuses a catalog that is not genuine code mode", () => {
        for (const declared of [undefined, new Set(), new Set(["exec", "exec_command"]), new Set(["exec", "shell_command"]), new Set(["apply_patch"])]) {
