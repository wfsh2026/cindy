...
import { toolChoiceCandidates, toolChoiceToolPredicate } from "../../../upstream/types";
test("allowed_tools resolves an ambiguous bare name without replaying the candidate list", () => {
    const size = 256;
    const backingTools = Array.from({ length: size }, (_, index) => ({
    ...
    Object.defineProperty(Array.prototype, Symbol.iterator, {
        ...arrayIterator,
        value: function () {
            const iterator = arrayIterator.value.call(this);
            ...
            return {
                next() {
                    ...
                    if (tracksCandidateList && !result.done)
                        ambiguousCandidateIterations += 1;
                    return result;
                },
                [Symbol.iterator]() {
                    return this;
    ...
    let filtered;
    try {
        const allowed = toolChoiceToolPredicate({ allowedTools: ["shared_name"], mode: "auto" }, tools);
...
test("public candidate lookups rebuild after a mutable caller changes its catalog", () => {
    const tools = [{ namespace: "first", name: "shared", description: "", parameters: {} }];
...
test("a compiled resolver fails closed when its catalog objects change", () => {
    const tools = [{ namespace: "stable", name: "shared", description: "", parameters: {} }];
    const allowed = toolChoiceToolPredicate({ allowedTools: ["shared"], mode: "auto" }, tools);
