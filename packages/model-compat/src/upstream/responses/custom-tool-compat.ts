...
export function collectRoutedCustomToolNames(
  ...
  const visit = (value: unknown): void => {
    ...
    // Only protocol containers contain declarations; schemas, metadata and tool payloads
    // can legitimately contain {type: "custom", name: ...} as literal user data.
    for (const key of ["tools", "input", "tool_choice"]) visit(value[key]);
  };
  visit(body);
  return names;
}
...
function collectConvertedCallIds(value: unknown, names: ReadonlySet<string>, out: Set<string>): void {
  ...
  if (Array.isArray(value.input)) collectConvertedCallIds(value.input, names, out);
}
...
function rewriteForUpstream(
  ...
  for (const [key, entry] of Object.entries(value)) {
    const rewritten = ["tools", "input", "tool_choice"].includes(key)
      ? rewriteForUpstream(entry, names, callIds) : entry;
    next[key] = rewritten;
