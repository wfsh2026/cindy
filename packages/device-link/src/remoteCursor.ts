...
export interface RemoteDesktopCursor {
  ...
  hotY: number;
  png: string;
  /** Optional native cursor hint. Unknown hints fall back to the local arrow. */
  shape?: string;
}
export interface RemoteDesktopCursorFrame {
  jpeg: string;
  cursor: RemoteDesktopCursor | null;
}
export function isRemoteDesktopCursor(
  value: unknown,
): value is RemoteDesktopCursor {
