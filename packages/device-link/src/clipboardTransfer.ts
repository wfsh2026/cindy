...
} from "./remoteClipboard.js";
import type { RemoteDesktopRequest } from "./remoteDesktop.js";
...
export async function transferClipboardContent(
  ...
  try {
    check();
    if (action === "paste") {
      const json = await clipboard.read();
      check();
      ...
      for (
        ...
        clipboard.progress?.(
          Math.min(json.length, offset + CLIPBOARD_CHUNK_CHARS),
          json.length,
        );
      }
      check();
      ...
      const result = await request<{ id: string; length: number }>({
        op: "clipboardContent",
        lease,
        action: "copy",
      });
      ...
      while (json.length < result.length) {
        ...
        if (
          typeof data !== "string" ||
          !data.length ||
          data.length > CLIPBOARD_CHUNK_CHARS ||
          json.length + data.length > result.length
        )
          throw new Error("CLIPBOARD_UNSUPPORTED");
        json += data;
        clipboard.progress?.(json.length, result.length);
      }
      check();
      parseClipboardContent(json);
