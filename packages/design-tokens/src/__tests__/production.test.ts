...
describe("DS-8 production source", () => {
  ...
  it("every modeled registration consumes generated defaults; all other IDs have explicit retention reasons", () => {
    ...
    visit(source);
    const registeredIdCount = new Set(ids).size;
    const bindingIdCount = Object.keys(bindings.defaults).length + Object.keys(bindings.retained).length;
    expect(registeredIdCount).toBe(bindingIdCount);
    expect(
      [
