...
it('unknown declarations never become permission items', () => {
  const result = validateGhostManifest({ ...base, cindy: { future: extension, image: ['future-action'] }, subscribe: { topics: ['future-topic'], hooks: ['future-hook'] } });
  expect(result.ok).toBe(true);
  if (!result.ok) return;
  expect(ghostPermissionItems(result.manifest).filter((item) => item.kind === 'cindy' || item.key.includes('future'))).toEqual([]);
});
