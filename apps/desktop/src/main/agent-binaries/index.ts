...
async function prepareViaCdn(
  ...
  if (!broadcastProgress) {
    const prepareOptions = { signal: opts.signal, checkForUpdates: opts.checkForUpdates };
...
export async function peekNeedsDownload(
  ...
  const primaryNeedsDownload = await primaryBase.peekNeedsDownload(opts);
  if (!shouldUseLegacyWindowsCodex(kind)) return primaryNeedsDownload;
  const legacyBase = getLegacyWindowsCodexBase();
  return legacyBase.peekNeedsDownload(opts);
}
