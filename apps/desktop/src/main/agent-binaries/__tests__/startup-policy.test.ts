...
describe('shared startup policy through the real binary preparation chain', () => {
  it.each([
    { checkForUpdates: false, broadcastProgress: false },
    { checkForUpdates: false, broadcastProgress: true },
    { checkForUpdates: true, broadcastProgress: false },
    { checkForUpdates: true, broadcastProgress: true },
  ])('preserves startup policy through the legacy Windows Codex fallback: %j', async (options) => {
    await reloadBinaries('win32');
    const installDir = path.join(mocks.userDataDir, 'codex', '1.0.0');
    const binaryPath = path.join(installDir, 'codex.exe');
    writeExecutable(binaryPath, 'codex-cli 1.0.0');
    const verifiedPath = path.join(installDir, '.verified');
    fs.writeFileSync(verifiedPath, '');
    const manifest = releaseManifest();
    const legacyAsset = { ...manifest.codexPackage!, file: 'codex/2.0.0/codex.exe.gz' };
    mocks.remoteManifest = { app: manifest.app, codex: legacyAsset };

    const needsDownload = await binaries.peekNeedsDownload('codex', options);
    expect(needsDownload).toBe(options.checkForUpdates);
    const result = await binaries.prepare('codex', options);
    const updatedPath = path.join(mocks.userDataDir, 'codex', '2.0.0', 'codex.exe');
    const expectedPath = options.checkForUpdates ? updatedPath : binaryPath;
    expect(result).toMatchObject({ ready: true, path: expectedPath });
    const downloadCount = options.checkForUpdates ? 1 : 0;
    expect(mocks.download).toHaveBeenCalledTimes(downloadCount);
  });
