 ...
 * CLI_COMMAND_NAME);`{{path}}` 替换为 symlink 安装路径。文案里的 “Cartethyia” 指产品/应用
 * 本身,不随命令名变化,故保留。
 */
...
export const APPLICATION_MENU_LABELS: Record<ApplicationMenuLocale, ApplicationMenuLabels> = {
  'zh-CN': {
    ...
    installCliConfirmDetail:
      '将在 {{path}} 创建指向 Cartethyia 的符号链接；若该路径已存在同名文件，会被替换。之后在终端运行 {{cmd}} . 即可把当前目录作为工作目录在 Cartethyia 中打开。系统会请求你的管理员密码以写入该目录。',
    ...
    installCliDevOnlyDetail:
      '开发模式下无法安装命令行工具：此时的可执行文件指向 Electron 解释器，而非 Cartethyia 应用。请在正式安装的 Cartethyia 中使用此功能。',
    installCliUnsupportedTitle: '暂不支持当前系统',
    installCliUnsupportedDetail: '安装到命令行目前仅支持 macOS。',
    installCliNotInApplicationsTitle: '请先把 Cartethyia 移动到「应用程序」文件夹',
    installCliNotInApplicationsDetail:
      '当前 Cartethyia 运行在临时位置（不在「应用程序」文件夹里）。此时安装的命令会指向这个临时路径，应用退出或磁盘弹出后即失效。请先把 Cartethyia 移动到「应用程序」文件夹，再安装命令行。',
  ...
  'zh-TW': {
    ...
    installCliConfirmDetail:
      '系統會在 {{path}} 建立指向 Cartethyia 的符號連結；如果該路徑已有同名檔案，將會取代它。之後在終端機執行 {{cmd}} .，即可將目前目錄作為工作目錄在 Cartethyia 中開啟。系統會要求你輸入管理員密碼，以便寫入該位置。',
    ...
    installCliDevOnlyDetail:
      '開發模式無法安裝命令列工具：此時可執行檔指向 Electron 執行環境，而不是 Cartethyia 應用程式。請在已安裝的 Cartethyia 中使用此功能。',
    installCliUnsupportedTitle: '目前系統不支援',
    installCliUnsupportedDetail: '目前僅 macOS 支援安裝命令列工具。',
    installCliNotInApplicationsTitle: '請先將 Cartethyia 移到「應用程式」檔案夾',
    installCliNotInApplicationsDetail:
      'Cartethyia 目前從暫存位置（不在「應用程式」檔案夾）執行。在此狀態下安裝的命令會指向暫存路徑，應用程式結束或磁碟退出後就會失效。請先將 Cartethyia 移到「應用程式」檔案夾，再安裝命令列工具。',
  ...
  en: {
    ...
    installCliConfirmDetail:
      'A symlink to Cartethyia will be created at {{path}}; if a file already exists there, it will be replaced. You can then run `{{cmd}} .` in a terminal to open the current directory as a working directory in Cartethyia. You will be asked for your administrator password to write to that location.',
    ...
    installCliDevOnlyDetail:
      'The command-line tool cannot be installed in development mode: the executable points at the Electron runtime rather than the Cartethyia app. Use this from an installed Cartethyia build.',
    ...
    installCliNotInApplicationsTitle: 'Move Cartethyia to the Applications folder first',
    installCliNotInApplicationsDetail:
      'Cartethyia is running from a temporary location (not the Applications folder). A command installed now would point at that temporary path and break as soon as the app quits or the volume is ejected. Move Cartethyia to the Applications folder, then install the command.',
  ...
  ja: {
    ...
    installCliConfirmDetail:
      'Cartethyia へのシンボリックリンクを {{path}} に作成します。そのパスに既存のファイルがある場合は置き換えられます。その後ターミナルで {{cmd}} . を実行すると、現在のディレクトリを作業ディレクトリとして Cartethyia で開けます。この場所への書き込みには管理者パスワードの入力を求められます。',
    ...
    installCliDevOnlyDetail:
      '開発モードではコマンドラインツールをインストールできません。実行ファイルが Cartethyia アプリではなく Electron ランタイムを指しているためです。インストール済みの Cartethyia から実行してください。',
    installCliUnsupportedTitle: 'このシステムには非対応',
    installCliUnsupportedDetail: 'コマンドラインへのインストールは現在 macOS のみ対応しています。',
    installCliNotInApplicationsTitle: 'まず Cartethyia を「アプリケーション」フォルダに移動してください',
    installCliNotInApplicationsDetail:
      'Cartethyia は一時的な場所(「アプリケーション」フォルダの外)で実行されています。この状態でインストールしたコマンドはその一時パスを指すため、アプリの終了やボリュームの取り出しで無効になります。Cartethyia を「アプリケーション」フォルダに移動してから、コマンドをインストールしてください。',
  ...
  ko: {
    ...
    installCliConfirmDetail:
      'Cartethyia를 가리키는 심볼릭 링크를 {{path}}에 만듭니다. 해당 경로에 파일이 이미 있으면 대체됩니다. 이후 터미널에서 {{cmd}} . 를 실행하면 현재 디렉터리를 작업 디렉터리로 Cartethyia에서 열 수 있습니다. 이 위치에 쓰려면 관리자 암호를 입력해야 합니다.',
    ...
    installCliDevOnlyDetail:
      '개발 모드에서는 명령줄 도구를 설치할 수 없습니다. 실행 파일이 Cartethyia 앱이 아닌 Electron 런타임을 가리키기 때문입니다. 설치된 Cartethyia에서 사용하세요.',
    installCliUnsupportedTitle: '이 시스템에서는 지원되지 않음',
    installCliUnsupportedDetail: '명령줄 설치는 현재 macOS에서만 지원됩니다.',
    installCliNotInApplicationsTitle: '먼저 Cartethyia를 응용 프로그램 폴더로 옮기세요',
    installCliNotInApplicationsDetail:
      'Cartethyia가 임시 위치(응용 프로그램 폴더 밖)에서 실행되고 있습니다. 지금 설치하는 명령은 이 임시 경로를 가리키므로 앱을 종료하거나 볼륨을 꺼내면 작동하지 않습니다. Cartethyia를 응용 프로그램 폴더로 옮긴 다음 명령을 설치하세요.',
