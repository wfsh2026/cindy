...
export const AutomationSessionGroupItem = withSidebarNavigation<AutomationSessionGroupItemProps>(
  function AutomationSessionGroupItem({
    navigate,
    group,
    activeSessionId,
    runningSessionIds,
    ...
    matchMap,
    sessionVariant = 'text',
  }: AutomationSessionGroupItemProps & SidebarNavigationProps) {
    ...
    const isRunning =
      latestSessionId != null && latestSession?.deviceLinkDeviceId
    ...
    const handleChildSessionClick = useCallback<SessionClickHandler>(
      (id, modifiers) => {
        if (
          ...
          !showAllRef.current &&
          !hasSessionSelectionModifier(modifiers)
        ) {
                  ...
                  {showRightStatus ? (
                    groupRightStatusKind === 'error' ? (
                      ...
                      <span
                        ...
                        className="inline-flex size-4 items-center justify-center"
                        aria-label={t(
                          'ccAgent.sidebar.status.needsAttention',
                          'Awaiting your input',
                        )}
